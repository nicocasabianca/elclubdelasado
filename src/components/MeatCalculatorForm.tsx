...
export const MeatCalculatorForm = ({
  ...
  const handleMeatTypeToggle = (type: string) => {
    setMeatTypes(prev => prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type]);
  };
  ...
  return <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input id="title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Ej: Asado familiar, Reunión de amigos..." />
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {totalParticipants > 0 && <Badge variant="secondary">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-2">
              ...
              <Input id="adults" type="number" min="0" value={adults} onChange={e => setAdults(parseInt(e.target.value) || 0)} />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="children">Niños (200g)</Label>
              <Input id="children" type="number" min="0" value={children} onChange={e => setChildren(parseInt(e.target.value) || 0)} />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="bigEaters">De buen comer (500g)</Label>
              <Input id="bigEaters" type="number" min="0" value={bigEaters} onChange={e => setBigEaters(parseInt(e.target.value) || 0)} />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="vegetarians">Vegetarianos (400g verduras)</Label>
              <Input id="vegetarians" type="number" min="0" value={vegetarians} onChange={e => setVegetarians(parseInt(e.target.value) || 0)} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-2">
              <Checkbox id="offal" checked={includeOffal} onCheckedChange={checked => setIncludeOffal(checked as boolean)} />
              <Label htmlFor="offal" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Incluir achuras (chinchulines, mollejas, etc.) - 10% del total</Label>
            </div>
            
            <div className="flex items-center space-x-2">
              <Checkbox id="longEvent" checked={longEvent} onCheckedChange={checked => setLongEvent(checked as boolean)} />
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4">
          <Button onClick={handleCalculate} disabled={meatTypes.length === 0 || totalParticipants === 0} className="flex-1">
            <Calculator className="h-4 w-4 mr-2" />
            Calcular Cantidades
          </Button>
          
          {currentResult && <Button onClick={handleSaveCalculation} disabled={loading} variant="outline">
